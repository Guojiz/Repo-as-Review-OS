...
本教程说明 GitLearnOS 在三个推荐运行层里的完整配置方式：
...
代码或 CLI 助手
→ Codex、Claude Code、Cursor 或其他仓库型工具
```

目标不是让所有平台变成同一种东西，而是让每个平台都遵守同一个 GitLearnOS 学习闭环：
...
```text
ChatGPT
→ 推荐作为日常学习平台，用于推理、写作、解释、记忆校准、GitHub 辅助工作和轻量自动化接手

Claude
→ 推荐作为日常学习平台，用于推理、写作、长上下文、项目式组织和 GitHub / MCP 辅助工作

OpenHanako / HanaAgent
→ 推荐作为桌面端增强运行时，用于本地文件、多 Agent、Skills、定时任务、浏览器 / 电脑操作和更深入的文件自动化

Codex / Claude Code / Cursor / CLI agents
→ 适合编码、仓库清理、脚本、批量修改和开发者工作流
```

ChatGPT 和 Claude 不默认写成桌面端运行时。它们应该直接使用自己的原生能力。
...
### 1. 创建 Claude Project 或等价工作区

只放稳定指令和少量参考文件。

不要把 Claude Project 当成学习仓库。
...
我的 GitHub 目标仓库是学习状态事实源。项目文件只作为参考材料。

行动前先判断你是否有 GitHub 权限、本地文件权限、记忆 / 项目指令支持、MCP / 工具和写入权限。

遵守这个闭环：
来源 → 模型 → 知识缺口 → 个性化题目 → 复习结果 → learner-profile.md → 下次复习。

除非你能说出具体改了哪些文件，否则不要声称已经编辑文件。
```

### 3. 连接 GitHub 或 MCP 后先确认边界

```text
你能读取我的 GitLearnOS 目标仓库吗？你能写入吗？你能使用 MCP 或连接器吗？你能访问本地文件吗？开始工作前先汇报边界。
```

### 4. 日常使用提示词

```text
请读取我的 GitLearnOS 状态：dashboard.md、learner-profile.md、当前目标、最近模型、knowledge-gaps/ 和 reviews/。

根据活跃知识缺口生成今天的下一步行动和一小套练习。每道题都要链接到来源、模型或学习画像条目。如果你不能读取仓库，请向我索要最小必要摘录。
```

### 5. 记忆行为

如果 Claude 有原生记忆或持久项目上下文，只保存稳定偏好和长期模式。

如果没有，就让变化的学习状态留在 GitHub，并在需要时索要仓库摘录。
...
Codex、Claude Code、Cursor 或其他 CLI / 代码 Agent 适合做仓库维护、脚本、清理和批量修改。

除非学习者偏好开发者工作流，否则不要把它们当默认日常学习导师。

推荐提示词：

```text
你正在维护一个 GitLearnOS 仓库。

除非用户明确要求，不要修改模板仓库。请在目标仓库工作。

检查 dashboard.md、learner-profile.md、sources/、models/、knowledge-gaps/、reviews/ 和 automations/。

做小而可审查的修改，并汇报每个被修改的文件。
```
