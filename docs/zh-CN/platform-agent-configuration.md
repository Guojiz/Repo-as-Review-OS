...
Claude、Codex、Claude Code、Cursor 和其他代码 / CLI Agent 原则上仍然可以读写 GitLearnOS 文件结构，但本仓库不详细维护它们的配置教程。偏好这些生态的用户，可以自行适配通用仓库协议，或使用其他专门的 Agentic tutoring / coding-agent 工作流。

目标不是让所有平台变成同一种东西，而是让本仓主动支持的平台遵守同一个 GitLearnOS 学习闭环：
...
```text
ChatGPT
→ 本仓详细支持的日常学习平台，用于推理、解释、写作、记忆校准、GitHub 辅助工作和轻量自动化接手

OpenHanako / HanaAgent
→ 本仓详细支持的桌面端增强运行时，用于本地文件、多 Agent、Skills、定时任务、浏览器 / 电脑操作和更深入的文件自动化
