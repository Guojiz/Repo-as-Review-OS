...
GitLearnOS 支持 skill 型 agent、记忆驱动的聊天应用、项目指令工作流、桌面多 Agent 运行时和单上下文 AI 工具。
...
```text
日常原生 AI 平台：ChatGPT / Claude
→ 推荐用于日常学习对话、推理、解释、写作、记忆校准、来源解读、GitHub 辅助工作和轻量复习生成

桌面端增强 Agent 运行时：OpenHanako / HanaAgent
→ 推荐给需要本地文件、多 Agent、记忆、Skills、定时任务、浏览器 / 电脑操作和更深入自动化的用户

代码 / CLI 助手层：Codex / Claude Code / Cursor / 仓库型 CLI agents
→ 适合编码、脚本、仓库清理、批量修改和开发者工作流

轻量单上下文层：智谱清言、国产单上下文 Agent、其他聊天式 AI
→ 适合在一个上下文里读取少量规则、材料和仓库片段，执行简化版学习工作流
```

默认不要求用户再额外配置桌面 Agent。ChatGPT 或 Claude 已经可以通过原生平台能力完成很多日常学习流程。OpenHanako 是当用户需要本地文件访问、多 Agent 协作、Skills 和电脑侧定时任务时的桌面增强推荐。
