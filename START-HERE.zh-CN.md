...
先读 START-HERE.zh-CN.md、README.zh-CN.md、QUICKSTART.zh-CN.md、docs/zh-CN/first-experiment-guide.md、docs/zh-CN/runtime-self-adaptation.md、OPERATING-MODEL.zh-CN.md、AGENT-RUNTIME.zh-CN.md、AGENTS.zh-CN.md、docs/zh-CN/skill-and-memory-runtime.md、docs/adaptive-memory-and-learner-profile.md 和 docs/lightweight-skg-dpm.md，再看 examples/zh-CN/demo-zhongkao-lite、examples/en/demo-sat-lite 和 examples/en/demo-research-reading-lite。
...
接着在我的目标仓库里创建目标文件、learner-profile.md、目录结构、来源规则、模型模板、知识缺口字段、间隔复习字段、自动化输出位置和控制台。每次创建或修改文件都要汇报。
```
...
4. 再读 `docs/zh-CN/first-experiment-guide.md`。
5. 再读 `docs/zh-CN/runtime-self-adaptation.md`。
6. 再读 `OPERATING-MODEL.zh-CN.md`。
7. 再读 `AGENT-RUNTIME.zh-CN.md`。
8. 再读 `AGENTS.zh-CN.md`。
9. 再读 `docs/zh-CN/skill-and-memory-runtime.md`。
10. 再读 `docs/adaptive-memory-and-learner-profile.md`。
11. 再读 `docs/lightweight-skg-dpm.md`。
12. 向用户解释这个系统。
13. 判断当前运行环境、记忆能力、文件访问和权限边界。
14. 区分模板仓库和目标学习仓库。
15. 指导用户创建或选择一个目标 GitHub 仓库。
16. 判断使用 skill 运行层、原生记忆、项目指令，还是单上下文回退方案。
17. 在目标仓库中搭建目录结构、模板、目标文件、学习画像、知识缺口、控制台和运行规则。
18. 汇报创建或修改了哪些文件。
