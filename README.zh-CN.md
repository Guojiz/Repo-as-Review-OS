...
**Learning Trace OS 是给你用于学习的人工智能大脑，可以让你的学习更加高效、智能。**

仓库名暂时仍为 `Repo-as-Review-OS`，用于保持链接连续；项目展示名改为 **Learning Trace OS**。
...
用户目标仓库
→ 个人 Learning Trace OS 仓库
→ 目标、学习画像、来源、模型、复习、dashboard
```
...
仓库名是 Repo-as-Review-OS，但项目概念名是 Learning Trace OS。

先读 QUICKSTART.zh-CN.md、FAQ.zh-CN.md、docs/zh-CN/30-second-intro.md、docs/zh-CN/first-experiment-guide.md、docs/zh-CN/what-goes-into-github.md、docs/zh-CN/runtime-self-adaptation.md、OPERATING-MODEL.zh-CN.md、START-HERE.zh-CN.md、AGENT-RUNTIME.zh-CN.md、AGENTS.zh-CN.md、skills/README.md、docs/zh-CN/skill-and-memory-runtime.md、docs/adaptive-memory-and-learner-profile.md、docs/lightweight-skg-dpm.md 和 docs/agentic-tutoring-standard.md。

帮我搭建自己的 Learning Trace OS。先解释它是什么，再判断你当前的运行环境、权限边界、记忆能力、GitHub 访问能力，以及哪些步骤需要我手动完成。
...
接着在我的目标仓库里创建目标文件、learner-profile.md、目录结构、来源规则、题目模型模板、间隔复习字段、自动化输出位置和控制台。每次创建或修改文件都要汇报。
```

## 什么内容放进 GitHub

GitHub 主要保存结构化学习记录：目标、学习画像、来源记录、可复用模型、复习题单、控制台、下次复习日期和 AI 接手记录。
...
动态学习者层
→ learner-profile.md、复习结果、AI 原生记忆、可选外部记忆工具
```
