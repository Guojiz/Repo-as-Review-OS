...
- **专家或贡献者：**先读 [Product Positioning](docs/product-positioning.md)、[Agentic Tutoring Standard](docs/agentic-tutoring-standard.md)、[Lightweight SKG and DPM](docs/lightweight-skg-dpm.md) 和 [Adaptive Memory and Learner Profile](docs/adaptive-memory-and-learner-profile.md)。
...
先读 START-HERE.zh-CN.md、README.zh-CN.md、QUICKSTART.zh-CN.md、AGENT-RUNTIME.zh-CN.md、AGENTS.zh-CN.md、docs/zh-CN/runtime-self-adaptation.md、docs/zh-CN/skill-and-memory-runtime.md、docs/adaptive-memory-and-learner-profile.md 和 docs/lightweight-skg-dpm.md。只在当前运行环境需要时继续读更深层链接。
...
港大等研究者提出的 DeepTutor 显示，将静态知识 grounding、动态学习者记忆和闭环辅导结合起来，可以让个性化辅导质量相对最强基线提升 **10.8%**，并让五个基础模型的通用推理平均提升 **28.6%**。GitLearnOS 不复刻这种重型平台，而是把同一类原则轻量化到 GitHub：来源、模型、知识缺口、学习画像、练习、复习和记忆同步都保存在学习者自己的仓库中，可检查、可迁移、可继续。见 arXiv:2604.26962。

## 标准

这个项目应该对标现代 Agent 化辅导系统，而不是普通笔记模板。

见 [Agentic Tutoring Standard](docs/agentic-tutoring-standard.md)。这个清单用于检查统一上下文、来源 grounding、学习者状态、出题闭环、批判修改、写作、可视化、复习、接手、可检查性和诚实运行边界。
...
- [docs/tutoring-benchmark-source-reference.md](docs/tutoring-benchmark-source-reference.md)：辅导评测来源参考
...
- [docs/agentic-tutoring-standard.md](docs/agentic-tutoring-standard.md)：Agent 化辅导标准
