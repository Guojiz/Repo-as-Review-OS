...
接着在我的目标仓库里创建目标文件、learner-profile.md、目录结构、来源规则、题目模型模板、知识缺口字段、间隔复习字段、自动化输出位置和控制台。每次创建或修改文件都要汇报。
```
...
近期开放研究显示，把来源 grounding、学习者记忆和闭环练习结合起来的个性化辅导系统，可以让个性化辅导指标平均提升 **10.8%**，并让通用 Agent 推理能力提升 **29.4%**。GitLearnOS 不复刻重型平台，而是把同一类原则轻量化到 GitHub：来源、模型、知识缺口、学习画像、练习、复习和记忆同步都保存在学习者自己的仓库中，可检查、可迁移、可继续。见 arXiv:2604.26962。
